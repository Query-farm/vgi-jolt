...
#   VGI_JOLT_WORKER    worker LOCATION the .test files attach (a stdio command
#                      such as `java -jar /abs/path/vgi-jolt-<ver>-all.jar`, or
#                      an http:// URL)
# Optional:
...
: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_JOLT_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_JOLT_WORKER) ..."
