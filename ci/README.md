# CI: the vgi-jolt worker integration suite
...
(`test/sql/*.test`) against the vgi-jolt VGI worker through the **real DuckDB
...
1. **Build the worker** — `./gradlew shadowJar` produces a self-contained fat
   JAR at `build/libs/vgi-jolt-<ver>-all.jar`. Its manifest sets `Add-Opens`, so
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   ...
   chosen transport, points `VGI_JOLT_WORKER` at it, **warms the extension cache
   once** (`INSTALL vgi FROM community;` + `INSTALL httpfs FROM core;` +
   `INSTALL json FROM core;`) so each test file's explicit `LOAD vgi;` /
   `LOAD json;` / injected `LOAD httpfs;` succeeds, then runs
...
`VGI_JOLT_WORKER` resolves to. `run-integration.sh` is parameterized by a
...
| `TRANSPORT`  | `VGI_JOLT_WORKER`       | how the worker is started | readiness gate |
|--------------|-------------------------|---------------------------|----------------|
...
VGI_JOLT_WORKER_JAR="$JAR" ci/run-integration.sh
